/**
   ...
   RGB : 측정된 제품의 색상을 표시
   적외선 센서 : 물품이 상자에 들어갔을 때 감지
   LED : 물품이 상자에 들어갔을 때 켜짐
   ...
   7. 물품이 상자에 들어가면 해당 상자의 적외선 센서가 감지하고 LED가 켜짐
*/
...
void loop() {
  ...
  if (digitalRead(PIN_IR) == HIGH) {                          // 적외선 센서는 물건 감지 시 LOW값을 전달. HIGH라는 것은 감지되지 않았음
    ...
    if (value == 2) {                                           // 빨간 색 제품
      ...
      servo.write(POS_GREEN);                                   // 서보모터를 초록 색 제품 분류방향으로 회전
    } else if (value == 6) {                                    // 파란 색 제품
      servo.write(POS_BLUE);                                    // 서보모터를 파란 색 제품 분류방향으로 회전
    }
